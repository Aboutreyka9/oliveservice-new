    ...
    <div class="content-wrapper" style="padding: 24px; width: 100%; max-width: 100%; box-sizing: border-box;">
      ...
      <div class="card-premium" style="background: #FFFFFF; border-radius: 16px; padding: 28px; border: 1px solid #E2E8F0; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.05), 0 8px 10px -6px rgba(15, 23, 42, 0.01); width: 100%; box-sizing: border-box; overflow: hidden;">
        <div style="width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch;">
          <table id="table-roles" class="table display nowrap" style="width:100%; max-width:100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #F8FAFC; text-align: left; color: #64748B; border-bottom: 2px solid #E2E8F0;">
                ...
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">Code Rôle</th>
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">Libellé du Rôle</th>
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">Groupe / Département</th>
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; text-align: center;">Utilisateurs</th>
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; text-align: center;">Permissions</th>
  ...
  var table = $('#table-roles').DataTable({
    ...
    columns: [
      ...
      { data: 'groupe', render: function(d, type, row) {
        ...
        return '<span style="color:#334155; font-weight:600;">' + (d || row.module || 'Général') + '</span>';
      }},
      { data: 'nb_users', width: '100px', className: 'text-center', render: function(d, type) {
        if (type !== 'display') return d || 0;
        return '<span style="font-weight:800; color:#1E3A5F; background:#E0F2FE; border:1px solid #BAE6FD; padding:4px 12px; border-radius:20px; font-size:11px;">' + (d || 0) + ' agent(s)</span>';
      }},
      { data: 'nb_permissions', width: '110px', className: 'text-center', render: function(d, type) {
        if (type !== 'display') return d || 0;
        return '<span style="font-weight:800; color:#059669; background:#ECFDF5; border:1px solid #A7F3D0; padding:4px 12px; border-radius:20px; font-size:11px;">' + (d || 0) + ' droit(s)</span>';
      }},
      { data: 'statut_role', width: '110px', className: 'text-center', render: function(d, type, row) {
        var isActif = (d === 'actif');
        var checkedAttr = isActif ? 'checked' : '';
        var canManage = (window.AppConfig && typeof window.AppConfig.can === 'function') ? window.AppConfig.can('ADMIN_MANAGE_ROLES') : false;
        ...
        return '<div style="display:flex; justify-content:center; align-items:center;">' +
               ...
               '<input type="checkbox" class="toggle-statut-role" data-id="' + row.id + '" ' + checkedAttr + ' ' + disabledAttr + ' style="opacity:0; width:0; height:0;">' +
