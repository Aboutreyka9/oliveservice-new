    ...
    <div class="content-wrapper" style="padding: 24px; width: 100%; max-width: 100%; box-sizing: border-box;">
      ...
      <div class="card-premium" style="background: #FFFFFF; border-radius: 16px; padding: 28px; border: 1px solid #E2E8F0; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.05), 0 8px 10px -6px rgba(15, 23, 42, 0.01); width: 100%; box-sizing: border-box; overflow: hidden;">
        <div style="width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch;">
          <table id="table-type-depense" class="table display nowrap" style="width:100%; max-width:100%; border-collapse: collapse; font-size: 13px;">
            <thead>
              <tr style="background: #F8FAFC; text-align: left; color: #64748B; border-bottom: 2px solid #E2E8F0;">
                ...
                <th style="padding: 14px 16px; font-weight: 800; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">Libellé</th>
...
$(document).ready(function() {
  var table = $('#table-type-depense').DataTable({
    ...
    columns: [
      ...
      { data: 'code_type_depense', width: '120px', render: function(d, type) {
      ...
      { data: 'libelle_type_depense', render: function(d, type) {
        if (type !== 'display') return d || '';
        return '<span style="font-weight:800; color:#0F172A; font-size:14px;">' + (d || '-') + '</span>';
      }},
      { data: 'description_type_depense', defaultContent: '-', render: function(d, type) {
        if (type !== 'display') return d || '';
        return '<span style="color:#64748B; font-weight:500;">' + (d || '-') + '</span>';
      }},
      { data: 'statut_type_depense', width: '110px', className: 'text-center', render: function(d, type, row) {
        var isActif = (d === 'actif');
        var checkedAttr = isActif ? 'checked' : '';
        var canManage = (window.AppConfig && typeof window.AppConfig.can === 'function') ? window.AppConfig.can('FINANCE_MANAGE_TYPE_DEPENSES') : false;
        ...
        return '<div style="display:flex; justify-content:center; align-items:center;">' +
               ...
               '<input type="checkbox" class="toggle-statut-td" data-id="' + row.id_type_depense + '" ' + checkedAttr + ' ' + disabledAttr + ' style="opacity:0; width:0; height:0;">' +
